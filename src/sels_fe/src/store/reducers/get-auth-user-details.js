...
const getAuthUserDetailsReducer = (state = initialState, action) => {
  switch (action.type) {
    case GET_AUTH_USER_DETAILS_SUCCESS:
      return getAuthUserDetailsSuccess(state, action)
    case GET_AUTH_USER_DETAILS_FAIL:
      return getAuthUserDetailsFail(state, action)
    default:
      return state
  }
}
